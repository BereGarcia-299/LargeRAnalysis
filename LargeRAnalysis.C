...
//std::vector<std::vector<std::vector<double>>> binsGenR;
 ...
 binsR10_Truth.emplace_back(std::vector<double>{177, 197, 220, 241, 262, 294, 336, 384, 439, 502, 573, 656, 750, 1000}); //70-80%
  ...
  TH1D *pTDisReco_All[totRadii][number_CentBins];
  ...
  if(jz_samples_pTDis){
	...
	if(full_and_ClsrTst){
	 ...
	 for(int iHalf =0; iHalf <  twoHalves; iHalf++){
	  ...
	  pTDisTruth_All[jet_Rad][iCent] = new TH1D(Form("R%d_Cent_%s_AllTruthJets",JetRadius[jet_Rad],centBin_strg.c_str()),Form("R%d_Cent_%s_AllTruthJets",JetRadius[jet_Rad],centBin_strg.c_str()),numpTBinsTruth,arrayTruth);
	  pTDisReco_All[jet_Rad][iCent] = new TH1D(Form("R%d_Cent_%s_AllRecoJets",JetRadius[jet_Rad],centBin_strg.c_str()),Form("R%d_Cent_%s_AllRecoJets",JetRadius[jet_Rad],centBin_strg.c_str()),numpTBinsTruth,arrayTruth);
	  if(debug)cout << __LINE__ << endl;
	}else if(!matchReq){
...
if(addpTShapeWght){
  ...
  for(int iEvent = 0; iEvent < events_tot; iEvent++){    
    ...
    if((collisionType == "PbPb") && (data_or_mc == mcOrdata::data) && (m_b_is_oo_pileup == 1 || m_b_is_pileup==1))continue;
	...
	if(matchReq){
	  ...
	  if(UniqueMatchToTrthJet[truth_index]==false){
		...
		if(!sumpTCut_MtchTrthJets){
		  ...
		  responseMatrix[jet_Rad][iCentBin]->Fill(akt_jet_kin[jet_Rad][pTPar]->at(iJetReco),akt_truth_jet_kin[jet_Rad][pTPar]->at(truth_index),centWeight*jZweight*ptShpWeight);//Resp. Mtx.Full Closure Test
		}
	...
	if(debug)cout << "This  is the iJetReco value: " << iJetReco << endl;
	  ...
	  for(int iCentBin =0; iCentBin < number_CentBins; iCentBin++){
		...
		if(akt_jet_kin[jet_Rad][pTPar]->at(iJetReco)< lowEndCutOff){
		  ...
		  ptShpWeight=ptshapeWghts[jet_Rad][centBin]->Eval(akt_jet_kin[jet_Rad][pTPar]->at(iJetReco)); 
		  
		} 
	      }

	      
	      pTDisReco_All[jet_Rad][iCentBin]->Fill(akt_jet_kin[jet_Rad][pTPar]->at(iJetReco),jZweight*ptShpWeight);
	    }
	  ...
	  if(data_or_mc == mcOrdata::mc){
	    ...
	    pTDisReco_All[jet_Rad][centBin]->Fill(akt_jet_kin[jet_Rad][pTPar]->at(iJetReco),centWeight*jZweight*ptShpWeight);
	  }
  ...
  TFile *files_root = new TFile(Form("%sRawHistograms_R%d_%s_%s.root",place_in_this_dir.c_str(),JetRadius[jet_Rad],collisionType.c_str(),extraTag.c_str()),"RECREATE");
   files_root->cd();     
	...
	pTDis[jet_Rad][iCentBin]->Write("",TObject::kOverwrite);
	  ...
	  pTDisReco_All[jet_Rad][iCentBin]->Write("",TObject::kOverwrite);
	  if(full_and_ClsrTst){
